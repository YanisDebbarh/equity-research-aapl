...
def get_calibration(ticker):
    try:
        ...
        info = dcf.info or {}
        raw  = hist.get('revenue_growth_hist', sect['growth'])
        growth = max(sect['growth'], raw) if raw > 0 else sect['growth']
        return {
            'growth'      : round(float(growth), 4),
            'ebit_margin' : round(float(max(sect['margin'], hist.get('ebit_margin_avg', sect['margin']))), 4),
        ...
        return {
            'growth': 0.07, 'ebit_margin': 0.15,
            'capex_pct': 0.05, 'tax_rate': 0.21,
            'tgr': 0.025, 'beta': 1.2,
            'sector': 'N/A', 'revenue_ltm': 0,
        }
...
with st.sidebar:
    ...
    g  = max(1, int(float(cal['growth']) * 100))
    g1 = st.slider("Y+1 (%)", 0, 40, g,             1) / 100
    g2 = st.slider("Y+2 (%)", 0, 40, g,             1) / 100
    g3 = st.slider("Y+3 (%)", 0, 30, max(1,int(g*0.9)), 1) / 100
    g4 = st.slider("Y+4 (%)", 0, 25, max(1,int(g*0.8)), 1) / 100
    g5 = st.slider("Y+5 (%)", 0, 20, max(1,int(g*0.7)), 1) / 100

    st.subheader("💰 Profitability")
    em     = max(1, int(float(cal['ebit_margin']) * 100))
    cx     = max(1, int(float(cal['capex_pct'])   * 100))
    ebit_m = st.slider("EBIT Margin (%)", 1, 60, em, 1) / 100
    capex  = st.slider("CapEx % Rev (%)", 1, 20, cx, 1) / 100

    st.subheader("📉 Discount Rate")
    tgr = st.slider("Terminal Growth (%)", 1, 5, max(1, int(cal['tgr']*100)), 1) / 100
...
""")
    ...
    st.warning("⚠️ Data loading... please wait or refresh.")

c1, c2, c3, c4 = st.columns(4)
c1.metric("Current Price", f"${current_price:.2f}")
c2.metric("DCF Target",    f"${price:.2f}", f"{upside:+.1f}%" if current_price else "")
...
if not hist.empty:
    ...
    fig.add_trace(go.Scatter(
        ...
        fill='tozeroy', line=dict(color='#3498db', width=2), name='Price'
    ))
    if price > 0:
...
if not proj.empty:
    ...
    for col in ['revenue_m','ebit_m','nopat_m','capex_m','ufcf_m','pv_ufcf_m']:
        if col in display.columns:
            display[col] = (display[col]/1000).round(2).astype(str) + 'B'
