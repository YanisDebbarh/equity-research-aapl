...
with st.sidebar:
    ...
    g = defaults['growth']
    g1 = st.slider("Growth Y+1", 0.0, 0.30, float(g),      0.01, format="%.0f%%")
    g2 = st.slider("Growth Y+2", 0.0, 0.30, float(g),      0.01, format="%.0f%%")
    g3 = st.slider("Growth Y+3", 0.0, 0.20, float(g*0.9),  0.01, format="%.0f%%")
    g4 = st.slider("Growth Y+4", 0.0, 0.20, float(g*0.8),  0.01, format="%.0f%%")
    g5 = st.slider("Growth Y+5", 0.0, 0.20, float(g*0.7),  0.01, format="%.0f%%")
    st.subheader("💰 FCF & Discount")
    fcf_m = st.slider("FCF Margin", 0.05, 0.40, float(defaults['fcf']),  0.01, format="%.0f%%")
    beta  = st.slider("Beta",       0.5,  2.5,  float(defaults['beta']), 0.05)
...
upside    = (dcf_price / current_price - 1) * 100 if current_price else 0
